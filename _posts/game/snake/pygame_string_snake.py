...
class snake():
    ...
    def design_food1(self):
        text = self.food_font.render("P", True, self.purple)
    ...
    def reset(self):
        ...
        self.food_x3 = round(random.randrange(0, self.window_width - self.snake_block) / 20.0) * 20.0
        self.food_y3 = round(random.randrange(0, self.window_height - self.snake_block) / 20.0) * 20.0
    ...
    def step(self,action):
        ...
        if self.snake_x == self.food_x3 and self.snake_y == self.food_y3:
            self.food_x3 = round(random.randrange(0, self.window_width - self.snake_block) / 20.0) * 20.0
            self.food_y3 = round(random.randrange(0, self.window_height - self.snake_block) / 20.0) * 20.0
            self.snake_length += 1
            reward = 10
